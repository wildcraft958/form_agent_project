...
class EnhancedFormProcessor:
    ...
    def process_form(self):
        ...
        print("\nBeginning form processing. Follow the prompts to complete the form.")
    ...
    def _process_sections(self, sections: List, parent_path: str = ""):
        ...
        for section in sections:
            ...
            # Only print section title if it has fields or subsections
            if section.get('fields') or section.get('sections'):
    ...
    def _should_skip_section(self, section: Dict) -> bool:
        ...
        if 'conditional' in section:
            dep_field_path = section['conditional']['depends_on']
            dep_field_value = None
            
            # Look up the dependent field value in form_data
            for path, value in self.form_data.items():
                if path.endswith(f"/{dep_field_path}") or path == dep_field_path:
                    if isinstance(value, dict) and 'value' in value:
                        dep_field_value = value['value']
                    else:
                        dep_field_value = value
                    break
            
            return dep_field_value not in section['conditional']['values']
        return False

    def _should_skip_field(self, field: Dict) -> bool:
        """Check field-level conditions"""
        if 'conditional' in field:
            dep_field_path = field['conditional']['depends_on']
            dep_field_value = None
            ...
            return dep_field_value not in field['conditional']['values']
        return False
    ...
    def _handle_field(self, field: Dict, field_path: str):
        ...
        print(f"Bot: Marking {field.get('label', field['name'])} for review")
        self._store_valid_input(field, field_path, "[NEEDS REVIEW]")
...
"""
    ...
    def _validate_field(self, field: Dict, value: str) -> Dict:
        ...
        # If standard validation passes but field has llm_validation flag, use LLM to validate complex medical content
...
"""
    ...
    def _validate_select(self, field: Dict, value: str) -> Dict:
        ...
        for opt in options:
            if value == opt.get('value') or value == opt.get('label'):
                if 'trigger' in opt:
                    self.context[opt['trigger']] = opt['value']
                return {"valid": True, "value": opt['value']}
                
        return {"valid": False, "message": f"Invalid selection. Choose from: {', '.join([opt.get('label', opt.get('value', '')) for opt in options])}"}
